    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (problemsResult.success) {
          ...
          // If no data exists, upload initial data (one-time setup)
          toast.loading('Setting up SIH data...', { duration: 3000 });
          const uploadResult = await uploadProblemStatements();
          if (uploadResult.success) {
            const retryResult = await fetchProblemStatements();
            if (retryResult.success) {
              setProblemStatements(retryResult.data);
              toast.success('SIH data loaded successfully!');
            }
          } else {
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-6">
          {filteredProblems.map((problem) => (
            <div key={problem.id} className="group bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/60 p-8 hover:shadow-3xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden relative">
